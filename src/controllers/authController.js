...
export const register = asyncHandler(async (req, res, next) => {
  ...
  const user = await StaffModel.create({ username, email, password, phone });
...
export const getMe = asyncHandler(async (req, res, next) => {
  const user = await StaffModel.findById(req.staff.id);
...
export const updateDetails = asyncHandler(async (req, res, next) => {
  ...
  const user = await StaffModel.findByIdAndUpdate(req.user.id, fieldsToUpdate, {
    new: true,
    runValidators: true,
  });
...
const sendTokenResponse = (user, statusCode, res) => {
  ...
  res
    ...
    .json({ success: true, data: { token, user } });
};
